<template>
    ...
    <v-dialog v-model="showResetDialog" max-width="400" persistent>
        <v-card>
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn variant="text" @click="confirmReset(false)">{{ languageStore.t('general.no') }}</v-btn>
                <v-btn variant="text" color="primary" @click="confirmReset(true)">{{ languageStore.t('general.yes') }}</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
</template>
