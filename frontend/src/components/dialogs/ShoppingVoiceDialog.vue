<template>
    <v-dialog v-model="dialogStore.dialogs.shoppingVoiceDialog" persistent>
        <v-card>
            ...
            <v-card-text>
                <div class="text-center py-4">
                    ...
                    <p class="mt-4" v-html="languageStore.t('shoppingList.voiceRecordingInfo')"></p>
                </div>
            </v-card-text>
            <v-card-actions>
...
const stopDialogWatch = watch(
    () => dialogStore.dialogs.shoppingVoiceDialog,
    async (newVal) => {
        if (newVal) {
...
const startRecording = async () => {
    try {
        ...
        dialogStore.closeDialog('shoppingVoiceDialog');
    }
};
